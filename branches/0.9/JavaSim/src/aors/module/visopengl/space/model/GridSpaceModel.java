...
import aors.module.visopengl.space.component.Cell;
import aors.module.visopengl.space.component.SpaceComponent;
...
import aors.module.visopengl.utility.Offset;
...
public class GridSpaceModel extends SpaceModel {
	...
	private void initialize() {
		...
		for (int row = 0; row < rows; row++) {
			for (int column = 0; column < columns; column++) {
				...
				cellOffset = new Offset(cellOffset.x2 - strokeWidth,
						cellOffset.y1, cellOffset.x2 - strokeWidth + cellDim,
						cellOffset.y2);
			}

			// Compute the offset for the first cell in a new row
			cellOffset = new Offset(firstCellOffset.x1, firstCellOffset.y1
					+ ((cellDim - strokeWidth) * (row + 1)),
					firstCellOffset.x2, firstCellOffset.y2
							+ ((cellDim - strokeWidth) * (row + 1)));
		}
	}
	...
	private double computeStrokeWidth(double cellDim) {
		...
		if (gridSpaceView.getAbsoluteStrokeWidth() != 0
			...
			strokeWidth = Math
					.ceil((gridSpaceView.getRelativeStrokeWidth() / 100)
							* cellDim);
		}
	 ...
	 * @param strokeWidth
	 */
	private Offset computeFirstCellOffset(int columns, int rows,
			double cellDim, double strokeWidth) {
		// Offsets
		double x1 = 0, y1 = 0;
		double x2 = 0, y2 = 0;
	...
	public void applyPropertyMaps() {
		if (generalSpaceModel != null) {
			if (aors.space.TwoDimensionalGrid.class
					.isInstance(generalSpaceModel.getSpace())) {
				...
				for (int row = 0; row < rows; row++) {
					for (int column = 0; column < columns; column++) {
						...
						if (abstractCell != null) {
							for (PropertyMap propertyMap : gridSpaceView
									.getPropertyMaps()) {
								try {
									// Get the property value through reflection
									Field property = abstractCell.getClass()
											.getDeclaredField(
													propertyMap
															.getPropertyName());
									...
									updateVisualProperty(propertyMap
											.getVisualPropertyName(),
											propertyMap.performMapping(property
													.get(abstractCell)),
											column, row);
							...
							for (SlotType slot : cell.getSlot()) {
								if (slot.getProperty().equals(
									...
									updateVisualProperty(
											...
											propertyMap.performMapping(slot
													.getValue()),
											(int) (cell.getX() - Space.ORDINATEBASE),
	...
	public void updateObjectsPerCell(Physical phy, int mode) {
		...
		if (index > spaceComponents.size() - 1) {
			return;
		}
		if (mode == 1) {
