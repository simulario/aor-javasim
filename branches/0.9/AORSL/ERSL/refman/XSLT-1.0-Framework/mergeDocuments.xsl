...
<!--
	...
	XML documents referenced by the base document.
...
<xsl:stylesheet
	...
	<!--
		...
		@param  maxDepth     - The maximum depth until the referenced documents
                           are merged. (optional)
    @param  relativeURIs - If true the URIs are treated relative, otherwise
                           (default) not 
	...
	<xsl:template name="x1f:mergeDocuments.mergeByNode">
		...
		<xsl:param name="maxDepth" select="''"/>
	  <xsl:param name="relativeURIs" select="false()"/>
		<xsl:param name="parameters"/>
		<xsl:if test="$documentNode">
			<xsl:call-template name="x1f:mergeDocuments.processDocument">
			  ...
			  <xsl:with-param name="relativeURIs" select="$relativeURIs"/>
				<xsl:with-param name="parameters" select="$parameters"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<!--
		...
		@param  listOfURIs   - The list of all known documents that shall be
                           ...
                           documents entry.
    @param  relativeURIs - If true the URIs are relative, otherwise not 
		@param  parameters   - A set of user-defined parameters for the document
                           processing.
	...
	<xsl:template name="x1f:mergeDocuments.processDocument">
	  ...
	  <xsl:param name="relativeURIs"/>
	    ...
	    <xsl:choose>
	      <xsl:when test="$relativeURIs = true()">
	        <xsl:call-template name="x1f:mergeDocuments.processDocument">
	...
	<!--
		...
		sorted like a tree linearization with beadth first search strategy).
	...
	<!--
		This template adds a set of new URIs to the given list of URIs.
		It appends the new values directly after the entry for the current documents
		URI (the list's entries will be sorted like a tree linearization with depth
