...
<!--
	This transformation creates classes for inMessages based on a given aorsml file.
	
	$Rev: 2586 $
	$Date: 2009-04-10 19:23:09 +0200 (Fri, 10 Apr 2009) $
...
<xsl:stylesheet version="2.0" xmlns:aorsml="http://aor-simulation.org" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:java="http://www.sun.com/java"
  ...
  <!--creates class-->
  <xsl:template match="aorsml:InMessageEventType" mode="createInMessages.createInMessage">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:call-template name="java:imports">
          <xsl:with-param name="importList" as="xs:string*">
            <xsl:value-of select="$core.package.inMessageEvent"/>
        ...
        <xsl:call-template name="java:class">
          ...
          <xsl:with-param name="name" select="@name"/>
          <xsl:with-param name="extends" select="if (fn:exists(@superType)) then @superType else $core.class.inMessageEvent"/>
          <xsl:with-param name="content">
            
            <!-- constructors -->
            <xsl:apply-templates select="." mode="createInMessages.constructors">
  ...
  <!-- constructors -->
  <xsl:template match="aorsml:InMessageEventType" mode="createInMessages.constructors">
    <xsl:param name="indent" required="yes" as="xs:integer"/>
    ...
    <!-- constructor (occurrenceTime, receiver, senderId, message) -->
    <xsl:call-template name="java:constructor">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="name" select="@name"/>
