...
<xsl:stylesheet version="2.0" xmlns:aorsml="http://aor-simulation.org" xmlns:fn="http://www.w3.org/2005/xpath-functions"
      ...
      <xsl:with-param name="content">
        ...
        <xsl:call-template name="java:class">
          ...
          <xsl:with-param name="content">
            ...
            <xsl:apply-templates select="." mode="createPhysicalObjects.constructor">
              <xsl:with-param name="indent" select="$indent + 1"/>
  ...
  <xsl:template name="setWidthAndHeightTo1">
    <xsl:param name="indent" as="xs:integer" required="yes"/>

    <xsl:call-template name="java:callSetterMethod">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="instVariable" select="'width'"/>
      <xsl:with-param name="value" select="'1'"/>
    </xsl:call-template>

    <xsl:call-template name="java:callSetterMethod">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="instVariable" select="'height'"/>
      <xsl:with-param name="value" select="'1'"/>
    </xsl:call-template>

  </xsl:template>

  <xsl:template name="createDefaultSettersForWidthAndHeight">
    <xsl:param name="indent" as="xs:integer" required="yes"/>

    <xsl:call-template name="java:inlineComment">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="content">
        <xsl:text>special case: twodimensionalGrid (or 2-dim discrete space)</xsl:text>
      </xsl:with-param>
    </xsl:call-template>
    <xsl:call-template name="java:method">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="modifier" select="'public'"/>
      <xsl:with-param name="name" select="'setWidth'"/>
      <xsl:with-param name="parameterList" as="xs:string*">
        <xsl:call-template name="java:createParam">
          <xsl:with-param name="type" select="jw:mappeDataType('double')"/>
    ...
    <xsl:call-template name="java:method">
      <xsl:with-param name="indent" select="$indent"/>
      <xsl:with-param name="modifier" select="'public'"/>
      <xsl:with-param name="name" select="'setHeight'"/>
      <xsl:with-param name="parameterList" as="xs:string*">
        <xsl:call-template name="java:createParam">
          <xsl:with-param name="type" select="jw:mappeDataType('double')"/>
