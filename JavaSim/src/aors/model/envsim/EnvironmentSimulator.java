...
public class EnvironmentSimulator implements EnvironmentAccessFacet {
  ...
  /**
   * This map contains objects with defined names; if the name is unique in the
   * simulation (...)
   */
  ...
  public EnvironmentSimulator(DataBusInterface dataBus, PhysicsSimulator physim) {
  ...
  public void run(long currentSimulationStep) {
    ...
    if (AbstractSimulator.runPhysics)
      physim.simulationStepExternal(currentEvents, currentSimulationStep, true);

    // add events that were generated by external simulators like the physics
    ...
    if (AbstractSimulator.runPhysics)
      physim
          .simulationStepExternal(currentEvents, currentSimulationStep, false);
  ...
  /**
   * Cycles through the list of global events (currentEvents) and processes all
   * events that are for current simulation step.
   */
    ...
    while (currentEventsIterator.hasNext()) {
      ...
      if (environmentEvent.getOccurrenceTime() == this.currentSimulationStep) {
        ...
        if (environmentEvent instanceof ExogenousEvent) {
          ...
          if (skip) {
            // if the stopCondition true, set the nextOccurrencetime to 0
            // to show in the logger, that we have no new occurrence time
      ...
      } // if (environmentEvent.getOccurrenceTime() == currentSimulationStep)
    }// while (currentEventsIterator.hasNext())
  ...
  private void processNonPerceptionEvent(EnvironmentEvent environmentEvent) {
    ...
    while (environmentRulesIterator.hasNext()) {
      ...
      ruleTriggeringEventSimpleName = environmentRule.getTriggeringEventType();
      ...
      if (currentEventSimpleName.equals(ruleTriggeringEventSimpleName)
        ...
        // event matching was performed and concrete instance is set to the rule
        environmentRule.setTriggeringEvent(environmentEvent);
        environmentRule.setTriggeredTime(environmentEvent.getOccurrenceTime());
        environmentRuleResult = environmentRule.execute();
        dataBus.notifyEnvEventResult(environmentRule, environmentRuleResult);
    ...
    // add all new events to newEvents list to later append it to global list
  ...
  private boolean checkMessageTypeForRulesTriggeredByOutMessageEvent(
    ...
    String eventMessageType = ((OutMessageEvent) environmentEvent).getMessage()
  ...
  private void computeAbstractSimState() {
    ...
    abstractSimState
        .setAgentsCount(this.aorObjectsByType.get(AOR_AGENT).size());
  ...
  private boolean addAORObject(Objekt objekt) {
    ...
    if (this.aorObjectsById.containsKey(objekt.getId())) {
      System.err.println("The ID " + objekt.getId() + " is allready used. The "
          + objekt.getClass() + "was not created.");
      return false;
    }
   ...
   * 
   * Comments: if the name are exists in the map, then the value will be set to
   * 'null'; this means the name is not unique in the system
   * 
   * @param objekt
   */
  ...
  private void addObjektToTypeMap(Class<? extends Objekt> clazz, Objekt objekt) {
    ...
    if (!(clazz.equals(PhysicalAgentObject.class)
        ...
        || clazz.equals(AgentObject.class) || clazz.equals(Objekt.class))) {
  ...
  public PhysicalAgentObject getPhysAgentById(long id, Class<?> type) {
    ...
    if (PhysicalAgentObject.class.isInstance(objekt) && type.isInstance(objekt))
   ...
   * 
   * 
   * Comments: search for an {@code object} in the list of {@code objects} with
   * specific {@code id}; if such an object is found then this {@code object} is
   * returned; otherwise it is returned {@code null}
   * 
   * @param id
   * @param type
  ...
  /**
   * 
   * Comments: This method searches the list of objects for an instance with the
   * given parameter {@code id}. If one is found, this instance is deleted from
   * the list and {@code true} is returned. Otherwise the content of list does
   * not change and {@code false} is returned by this method.
   * 
   * 
   * @param id
   */
  private boolean destroyAORObject(long id) {
    ...
    if (AgentObject.class.isInstance(objekt)) {
      // the remove will be executed in the following method for AgentObjects
  ...
  private void deleteObjektFromTypeMap(Class<? extends Objekt> clazz,
    ...
    if (this.aorObjectsByType.containsKey(objType)) {
      ...
      // System.out.println("REMOVE " + objekt.getType() + " from " + objType);
    }
   ...
   * 
   * 
   * Comments: This method returns an {@code ArrayList} of agent objects of the
   * given parameter {@code type}. The {@code ArrayList} is empty if there no
   * matching agent objects.
   * 
   * @param type
   ...
   * 
   */
  private void notifyDestroyEventListener(ObjektDestroyEvent objektDestroyEvent) {
  ...
  private EnvironmentEvent messageMapping(OutMessageEvent outMessageEvent) {
    ...
    return new InMessageEvent(this.currentSimulationStep, outMessageEvent
        .getReceiverIdRef(), outMessageEvent.getActorIdRef(), outMessageEvent
        .getMessage());
  }
