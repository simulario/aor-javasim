...
public class UpdateManager {
  ...
  // Map containing newly created object views and the step they where created in
  ...
  public void destroyObject(Object source, long currentSimulationStep) {
    if (source instanceof Objekt) {
      ...
      // Add destroyed objects IDs together with the step it was destroyed into a map  
  ...
  /**
   * Checks a number of simulation steps for updates and processes them. Returns
   * the number of the last simulation step that was read.
   * 
   * @param steps
   * @param buffer
   */
  public long readSimulationSteps(int steps, Queue<SimulationStep> buffer) {
    ...
    for (int i = 0; i < steps; i++) {
      ...
      // Make all objects visible that where created at this simulation step
