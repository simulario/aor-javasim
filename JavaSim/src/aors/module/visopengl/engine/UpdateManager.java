...
public class UpdateManager {
	 ...
	 * and stores a local copy of them inside of a hash map. Also the view that
	 * is associated to an objects ID is loaded.
	 * 
	 * @param initialState
	 * @param reader
	 */
	public void initializeObjects(InitialState initialState, XMLReader reader) {
		...
		for (Objekt obj : objList) {
			...
			if (obj instanceof PhysicalAgentObject) {
				PhysicalAgentObject physAgt = ((PhysicalAgentObject) obj)
						.clone();
	...
	/**
	 * When an object is created during runtime, a local copy of it is stored
	 * and its view is loaded.
	 * 
	 * @param source
	...
	private void loadObjView(long id, String type, XMLReader reader) {
		...
		/*
		 * When a view node was found, read the actual view definition and store
		 * it together with the objects ID in a hash map.
		 */
		if (node != null) {
			...
			if (view.getEmbeddedList() != null) {
				...
				for (View embeddedView : view.getEmbeddedList()) {
					if (embeddedView.getShape2DMap() != null) {
						embeddedView.getShape2DMap().determineShape(obj,
								embeddedView);
					}

					if (embeddedView.getEmbeddedList() != null) {
						for (View embeddedView2 : embeddedView
								.getEmbeddedList()) {
							if (embeddedView2.getShape2DMap() != null) {
								embeddedView2.getShape2DMap().determineShape(
										obj, embeddedView2);
							}
						}
					}
	...
	private void getShape(ObjectType obj) {
		...
		if (view != null) {
			...
			if (view.getEmbeddedList() != null) {
				for (View embeddedView : view.getEmbeddedList()) {
					if (embeddedView.getShape2DMap() != null) {
						embeddedView.getShape2DMap().determinePropertyValue(
								obj, embeddedView);
					}
	...
	private void getInitialShapeProperties(Objekt obj) {
		...
		if (view != null) {
			...
			if (view.getShape2DMap() != null) {
				Collection<Shape2D> collection = view.getShape2DMap().getMap()
						.values();
			...
			if (view.getEmbeddedList() != null) {
				for (View embeddedView : view.getEmbeddedList()) {
					if (embeddedView.getShape2DMap() != null) {
						for (Shape2D shape : embeddedView.getShape2DMap()
								.getMap().values()) {
							shape.applyPropertyMaps(obj);
						}
						...
						if (embeddedView.getEmbeddedList() != null) {
							for (View embeddedView2 : embeddedView
									.getEmbeddedList()) {
								if (embeddedView2.getShape2DMap() != null) {
									for (Shape2D shape : embeddedView2
											.getShape2DMap().getMap().values()) {
										shape.applyPropertyMaps(obj);
									}
								}
							}
						...
						if (embeddedView.getEmbeddedList() != null) {
							for (View embeddedView2 : embeddedView
								...
								if (embeddedView2.getShape() != null) {
									embeddedView2.getShape().applyPropertyMaps(
											obj);
								}
							}
						}
	...
	private void getObjectUpdates(AgentSimResultingStateChanges change) {
		...
		if (change.getPhysicalAgents() != null) {
			if (change.getPhysicalAgents().getPhysAgt() != null) {
				List<PhysAgtType> list = change.getPhysicalAgents()
						.getPhysAgt();
		...
		if (change.getPhysicalObjects() != null) {
			if (change.getPhysicalObjects().getPhysObj() != null) {
				List<PhysicalObjType> list = change.getPhysicalObjects()
						.getPhysObj();
	...
	private void updatePhysicalAgent(PhysAgtType phyAgt) {
		...
		PhysicalAgentObject obj = (PhysicalAgentObject) objMap.get(phyAgt
				.getId());
	...
	/**
	 * Checks a number of simulation steps for updates and processes them.
	 * Returns the number of the last simulation step that was read.
	 * 
	 * @param steps
	 * @param buffer
	 */
	public long readSimulationSteps(int steps, Queue<SimulationStep> buffer) {
		long simulationStepNumber = 0;

		for (int i = 0; i < steps; i++) {
			// Remove the first element from the buffer
			SimulationStep simStep = buffer.poll();
			...
			if (simStep.getAgentSimResultingStateChanges() != null) {
				...
				getGridPropertyChanges(simStep
						.getAgentSimResultingStateChanges());
			...
			simulationStepNumber = simStep.getStepTime();
		}

		return simulationStepNumber;
	}
	...
	private void updateFromPhysicsSimulation(PhysicsSimulationType phySim) {
		if (phySim.getResultingStateChanges() != null) {
			for (ResultingStateChanges change : phySim
		...
		for (JAXBElement<? extends EnvSimInputEventType> envSimInputEventType : envSimInputEvents) {
			if (envSimInputEventType.getValue() != null) {
				...
				if (envSimInputEventType.getValue().getActivities() != null) {
					...
					updateFromActivities(envSimInputEventType.getValue()
							.getActivities());
				}
			}
		}
	...
	private void updateFromActivities(Activities activities) {
		if (activities.getStartActivity() != null) {
			for (StartActivity startActivity : activities.getStartActivity()) {
				if (startActivity.getResultingStateChanges() != null) {
					// Get grid property changes
					getGridPropertyChanges(startActivity
							.getResultingStateChanges());
		...
		if (activities.getFinalizeActivity() != null) {
			for (FinalizeActivity finalizeActivity : activities
					.getFinalizeActivity()) {
				if (finalizeActivity.getResultingStateChanges() != null) {
					// Get grid property changes
					getGridPropertyChanges(finalizeActivity
							.getResultingStateChanges());

					// Update objects
					getObjectUpdates(finalizeActivity
							.getResultingStateChanges());
				}
			}
		}
