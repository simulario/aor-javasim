package aors.module.visopengl.xml;
...
import aors.module.visopengl.shape.Circle;
import aors.module.visopengl.shape.DisplayInfo;
import aors.module.visopengl.shape.Ellipse;
import aors.module.visopengl.shape.PolyLine;
import aors.module.visopengl.shape.Polygon;
import aors.module.visopengl.shape.Positioning;
import aors.module.visopengl.shape.Rectangle;
...
import aors.module.visopengl.space.view.GridSpaceView;
...
import aors.module.visopengl.space.view.TwoDimSpaceView;
...
public class XMLReader {
			...
			if (ui.getLength() > 0) {
				...
				for (int i = 0; i < uiChildNodes.getLength(); i++) {
					if (uiChildNodes.item(i).getNodeName().equals(ANIMATION_UI)) {
						...
						NodeList auiChildNodes = uiChildNodes.item(i)
								.getChildNodes();

						// Search for the "Views" node inside "AnimationUI"
						for (int j = 0; j < auiChildNodes.getLength(); j++) {
							if (auiChildNodes.item(j).getNodeName().equals(
									VIEWS)) {
	...
	public SpaceView getSpaceView(SpaceType spaceType) {
		...
		if (views != null) {
			...
			for (int i = 0; i < viewsChildNodes.getLength(); i++) {
				if (viewsChildNodes.item(i).getNodeName().equals(
						SpaceView.SPACE_VIEW)) {
					// Retrieve the attributes of a "SpaceView" node
					NamedNodeMap attributes = viewsChildNodes.item(i)
							.getAttributes();
					...
					for (int k = 0; k < attributes.getLength(); k++) {
						...
						else if (name.equals(SpaceView.CANVAS_COLOR_RGB)
								&& !colorSet) {
							canvasColor = new Color(value);
						}
					}
					...
					for (int j = 0; j < spaceViewChildNodes.getLength(); j++) {
						/*
						 * When the space view node matches the space models
						 * space type, it will be associated to the space model.
						 */
						if (spaceViewChildNodes.item(j).getNodeName().equals(
								OneDimSpaceView.ONE_DIMENSIONAL)) {
							if (spaceType.equals(SpaceType.OneD)) {
								spaceView = getOneDimSpaceView(spaceViewChildNodes
										.item(j));
							}
						}
							...
							if (spaceType.equals(SpaceType.TwoDGrid)) {
								spaceView = getGridSpaceView(spaceViewChildNodes
										.item(j));
								spaceView
										.setPropertyMaps(getPropertyMaps(spaceViewChildNodes
												.item(j)));
							}
						}
							...
							if (spaceType.equals(SpaceType.TwoD)
									|| spaceType
											.equals(SpaceType.TwoDLateralView)) {
								spaceView = getTwoDimSpaceView(spaceViewChildNodes
										.item(j));
							}
						}
					}
	...
	private SpaceView getOneDimSpaceView(Node node) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (name.equals(OneDimSpaceView.MODE))
			...
			else if (name.equals(OneDimSpaceView.TRACK_COLOR_RGB)
					&& !trackColorSet) {
				spaceView.setTrackColor(new Color(value));
			}
				...
				if (value.contains("%")) {
					spaceView.setRelativeTrackWidth(Double.valueOf(value
							.substring(0, value.length() - 1)));
				} else if (value.contains("px")) {
					spaceView.setAbsoluteTrackWidth(Double.valueOf(value
	...
	private SpaceView getGridSpaceView(Node node) {
		...
		/*
		 * Retrieve the name and value of each attribute and initializes the
		 * class members accordingly.
		 */
		for (int i = 0; i < attributes.getLength(); i++) {
			String name = attributes.item(i).getNodeName();
			...
			if (name.equals(GridSpaceView.FILL1)) {
				...
				if (value.contains("%")) {
					spaceView.setRelativeStrokeWidth(Double.valueOf(value
							.substring(0, value.length() - 1)));
				} else if (value.contains("px")) {
					spaceView.setAbsoluteStrokeWidth(Double.valueOf(value
							.substring(0, value.length() - 2)));
				}
			}
	...
	private SpaceView getTwoDimSpaceView(Node node) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			else if (name.equals(TwoDimSpaceView.BORDER_COLOR_RGB)
					&& !borderColorSet) {
				spaceView.setBorderColor(new Color(value));
			}
		}
	...
	public Map<String, Node> readViews() {
		...
		if (views != null) {
			...
			for (int k = 0; k < childNodes.getLength(); k++) {
				...
				if (childNodes.item(k).getNodeName().equals(
						View.PHYSICAL_OBJECT_VIEW)) {
	...
	/**
	 * Adds an object view node into a hash map together with its associated ID
	 * or type.
	 * 
	 * @param node
	 * @param map
	...
	private Shape2D readShape2D(Node node) {
		...
		for (int j = 0; j < childNodes.getLength(); j++) {
			...
			// Found "RegularPolygon" node
			else if (childNodes.item(j).getNodeName().equals(
					Shape2D.REGULAR_POLYGON)) {
				// Read all data from "RegularPolygon" node
		...
		if (shape != null) {
			...
			for (int j = 0; j < attributes.getLength(); j++) {
				...
				if (name.equals(Shape2D.X)) {
					...
					if (value.contains("%")) {
						shape.setX(Double.valueOf(value.substring(0, value
								.length() - 1)));
					...
					// Check if the position is relative or absolute
					if (value.contains("%")) {
						shape.setY(Double.valueOf(value.substring(0, value
								.length() - 1)));
	...
	private void readShapeAttributes(Node node, Shape2D shape) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (name.equals(Shape2D.FILL)) {
				...
				if (value.contains("%")) {
					shape.setWidth(Double.valueOf(value.substring(0, value
							.length() - 1)));
				...
				if (value.contains("%")) {
					shape.setHeight(Double.valueOf(value.substring(0, value
							.length() - 1)));
				...
				if (value.contains("%")) {
					double radius = Double.valueOf(value.substring(0, value
							.length() - 1));
		...
		if (childNodes.getLength() > 0) {
			for (int i = 0; i < childNodes.getLength(); i++) {
				...
				if (childNodes.item(i).getNodeName().equals(
					...
					/*
					 * Retrieve the attributes of a
					 * GridCellPropertyVisualizationMap or ShapePropertyMap
					 * node.
					 */
					NamedNodeMap attributes = childNodes.item(i)
							.getAttributes();

					/*
					 * Retrieve the name and value of each attribute and
					 * initialize the class members accordingly.
					 */
					for (int j = 0; j < attributes.getLength(); j++) {
						String name = attributes.item(j).getNodeName();
						String value = attributes.item(j).getNodeValue();
	...
	private View getEmbeddedShape(Node node) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (name.equals(Shape2D.OFFSET_X)) {
				if (value.contains("%")) {
					offsetX = Double.valueOf(value.substring(0,
							value.length() - 1));
				...
				if (value.contains("%")) {
					offsetY = Double.valueOf(value.substring(0,
							value.length() - 1));
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			if (childNodes.item(i).getNodeName().equals(
					Shape2D.PHYSICAL_SHAPE_2D)) {
				embeddedView
						.setShape2D(readPhysicalShape2D(childNodes.item(i)));
			}
		...
		if (embeddedView.getShape2DMap() != null) {
			Collection<Shape2D> collection = embeddedView.getShape2DMap()
					.getMap().values();
	...
	private Shape2DMap readShape2DMap(Node node) {
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			if (childNodes.item(i).getNodeName().equals(Shape2DMap.CASE)) {
				...
				NamedNodeMap caseAttributes = childNodes.item(i)
						.getAttributes();

				for (int j = 0; j < attributes.getLength(); j++) {
					// Get attribute name and value
					String attributeName = caseAttributes.item(j).getNodeName();
					String attributeValue = caseAttributes.item(j)
							.getNodeValue();

					if (attributeName.equals(Shape2DMap.VALUE)) {
						if (attributeValue.matches("[\\d]+")) {
							propertyValue = String.valueOf(Double
	...
	/**
	 * Get the simulation description content that is used for showing some
	 * infos in the visualization tab.
	 * 
	...
	public String getSimulationDescriptionInfo() {
		...
		if (descNodes != null && descNodes.getLength() > 0) {
			...
			try {
				descriptionContent = serializeNode(childDescNode, "");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	 ...
	 * <code>String</code>. Please note that is a special serialization that
	 * drops any used name-spaces and also do not parse special elements such as
	 ...
	 *            current indentation.
	 */
	public String serializeNode(Node node, String indentLevel)
			throws IOException {
		...
		case Node.ELEMENT_NODE:
			...
			if (children != null) {
				...
				for (int i = 0; i < children.getLength(); i++) {
					result += serializeNode(children.item(i), indentLevel
							+ indent);
				}
				if ((children.item(0) != null)
