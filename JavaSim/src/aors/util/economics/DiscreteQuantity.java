...
/**
 * The datatype class DiscreteQuantity allows to define discrete quantities (such
 * as 1 tractor or 3 cars)
 * 
...
public class DiscreteQuantity extends Quantity {
  ...
  public void deposit(double q) {
    if (q < 0)
      throw new RuntimeException(
          "Quantity to be deposited must be greater than zero");
    quantity += q;
  }

  /**
   * Remove a certain quantity of this type of object. If quantity to be
   * withdrawn is smaller than quantity, quantity is set to 0. Returns the
  ...
  public double withdraw(double q) {
    if (q < 0)
      throw new RuntimeException(
          "Quantity to be withdrawn must be greater than zero");
    if (q <= quantity) {
      quantity -= q;
      return q;
    } else {
