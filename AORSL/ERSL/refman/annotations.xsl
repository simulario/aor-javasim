...
<xsl:stylesheet version="1.0" exclude-result-prefixes="xs xsl x1f" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:x1f="http://www.informatik.tu-cottbus.de/~tgrundm1/projects/xslt-framework/xslt-1.0/">
	...
	<xsl:template name="getMemberTypesAnnotations">
		...
		<xsl:if test="normalize-space($currentType) != ''">
			<xsl:apply-templates select="$documentNodes/xs:simpleType[@name = $currentType or @name = substring-after($currentType,':')]" mode="annotations">
	...
	<xsl:template match="xs:element[@name]" mode="annotations">
		...
		<xsl:variable name="annotations">
			...
			<xsl:choose>
				<xsl:when test="@type">
					<xsl:apply-templates select="$documentNodes/xs:complexType[@name = current()/@name or @name = substring-after(current()/@name,':')] | $documentNodes/xs:simpleType[@name = current()/@name or @name = substring-after(current()/@name,':')]" mode="annotations">
						<xsl:with-param name="documentNodes" select="$documentNodes"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="xs:attribute[@name]" mode="annotations">
		...
		<xsl:variable name="annotations">
			...
			<xsl:choose>
				<xsl:when test="@type">
					<xsl:apply-templates select="$documentNodes/xs:simpleType[@name = current()/@type or @name = substring-after(current()/@type,':')]" mode="annotations">
						<xsl:with-param name="documentNodes" select="$documentNodes"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="xs:complexContent/xs:extension | xs:complexContent/xs:restriction" mode="annotations">
		...
		<xsl:apply-templates select="$documentNodes/xs:complexType[@name = current()/@base or @name = substring-after(current()/@base,':')]" mode="annotations">
	...
	<xsl:template match="xs:simpleContent/xs:extension | xs:simpleContent/xs:restriction" mode="annotations">
		...
		<xsl:apply-templates select="$documentNodes/xs:simpleType[@name = current()/@base or @name = substring-after(current()/@base,':')]" mode="annotations">
			<xsl:with-param name="level" select="$level + 1"/>
			<xsl:with-param name="documentNodes" select="$documentNodes"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="xs:list" mode="annotations">
		...
		<xsl:choose>
			<xsl:when test="@itemType">
				<xsl:apply-templates select="$documentNodes/xs:simpleType[@name = current()/@itemType or @name = substring-after(current()/@itemType,':')]" mode="annotations">
	...
	<xsl:template match="xs:simpleType/xs:restriction" mode="annotations">
		...
		<xsl:apply-templates select="$documentNodes/xs:simpleType[@name = current()/@base or @name = substring-after(current()/@base,':')]" mode="annonations">
			<xsl:with-param name="level" select="$level + 1"/>
			<xsl:with-param name="documentNodes" select="$documentNodes"/>
		</xsl:apply-templates>
			...
			<xsl:when test="local-name() = 'a' and @href and starts-with(@href,'#')">
				<xsl:variable name="id">
					<xsl:call-template name="x1f:createID">
						<xsl:with-param name="node" select="$documentNodes//xs:*[@id = substring-after(current()/@href,'#')]"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:choose>
