...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  xmlns:x1f="http://www.informatik.tu-cottbus.de/~tgrundm1/projects/xslt-framework/xslt-1.0/" version="1.0" exclude-result-prefixes="xs xsl x1f">
  ...
  <xsl:template name="addSeperator">
    ...
    <xsl:param name="condition"
      select="count($node/following-sibling::xs:sequence|$node/following-sibling::xs:choice|$node/following-sibling::xs:all|$node/following-sibling::xs:element|$node/following-sibling::xs:group) > 0"/>
  ...
  <xsl:template name="getType">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="concat('ANONYMOUS_TYPE:',generate-id($node))"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
    ...
    <xsl:if test="$hasNext = 'true'">
      ...
      <xsl:call-template name="x1f:createChapter">
        ...
        <xsl:with-param name="body">
          <xsl:apply-templates select="xs:element[not(@abstract = 'true') and (starts-with(@name,$value_upperCase) or starts-with(@name,$value_lowerCase))]"
            mode="elements">
  ...
  <xsl:template match="xs:element[@name]" mode="elements">
    ...
    <xsl:if test="$alreadyKnownLocalType = 'false'">
      ...
      <xsl:choose>
        <xsl:when test="@type">
          <xsl:apply-templates select="/xs:schema/xs:complexType[@name = current()/@type or @name = substring-after(current()/@type,':')]"
  ...
  <xsl:template match="xs:complexType" mode="elements.children">
    ...
    <xsl:apply-templates
      select=".//xs:element[@name and contains($candidateElements,concat(@name,' '))] |
      /xs:schema/xs:group[contains($candidateGroups,concat(@name,' '))]//xs:element[@name and contains($candidateElements,concat(@name,' '))] |
      /xs:schema/xs:complexType[contains($candidateTypes,concat(@name,' '))]//xs:element[@name and contains($candidateElements,concat(@name,' '))]"
      mode="elements.children">
      <xsl:sort select="@name"/>
      <xsl:with-param name="prefix" select="$prefix"/>
  ...
  <xsl:template match="xs:group[@ref]" mode="candidates">
    <xsl:param name="mode"/>
    <xsl:param name="listOfKnownGroups"/>
    ...
    <xsl:apply-templates select="/xs:schema/xs:group[@name = $ref]" mode="candidates">
      <xsl:with-param name="mode" select="$mode"/>
      <xsl:with-param name="listOfKnownGroups" select="$listOfKnownGroups"/>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="xs:group[@name]" mode="candidates">
    ...
    <xsl:variable name="isAlreadyKnown">
      <xsl:call-template name="x1f:List.containsValue">
        <xsl:with-param name="list" select="$listOfKnownGroups"/>
        <xsl:with-param name="value" select="@name"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:if test="$isAlreadyKnown = 'false'">
      <xsl:if test="$mode = 'groups'">
        <xsl:value-of select="concat(@name,' ')"/>
      </xsl:if>
      <xsl:apply-templates select="xs:sequence | xs:choice | xs:all" mode="candidates">
        <xsl:with-param name="mode" select="$mode"/>
        <xsl:with-param name="listOfKnownGroups">
  ...
  <xsl:template match="xs:complexContent/xs:extension | xs:complexContent/xs:restriction" mode="candidates">
    ...
    <xsl:apply-templates
      select="/xs:schema/xs:complexType[@name = current()/@base or @name = substring-after(current()/@base,':')] | xs:sequence | xs:choice | xs:all | xs:group"
      mode="candidates">
      <xsl:with-param name="mode" select="$mode"/>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="xs:simpleContent" mode="candidates">
    <xsl:param name="mode"/>
    <xsl:param name="listOfKnowGroups" select="''"/>
  </xsl:template>
  ...
  <xsl:template match="xs:element[@name and not(@abstract = 'true')]" mode="subelements">
    ...
    <xsl:variable name="substitutes">
      <xsl:variable name="candidates"
        select="/xs:schema/xs:element[@substitutionGroup = current()/@name or substring-after(@substitutionGroup,':') = current()/@name]"/>
  ...
  <xsl:template match="xs:schema/xs:element[@name and not(@abstract = 'true')]" mode="substitution.elements">
    ...
    <xsl:variable name="nextSiblings"
      select="following-sibling::xs:element[@substitutionGroup = current()/@substitutionGroup or @substitutionGroup = substring-after(current()/@substitutionGroup,':') or substring-after(@substitutionGroup,':') = current()/@substitutionGroup or substring-after(@substitutionGroup ,':')= substring-after(current()/@substitutionGroup,':')]"/>
