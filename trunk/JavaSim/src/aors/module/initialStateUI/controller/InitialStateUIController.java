...
public class InitialStateUIController implements Module {
    ...
    for (int i = 0; i < childrenNodeList.getLength(); i++) {
      ...
      if (child instanceof Element) {
        ...
        if (nodeName.equalsIgnoreCase(XMLConstants.OBJECT_UI)) {
          ...
          typeName = typeNameElement.getAttributeNode(XMLConstants.OBJECT_TYPE)
              .getValue();
          ...
          typeName = typeNameElement.getAttributeNode(XMLConstants.AGENT_TYPE)
              .getValue();
          initialStateUIType.setTypeName(typeName);
          ...
          typeName = typeNameElement.getAttributeNode(XMLConstants.EVENT_TYPE)
              .getValue();
          initialStateUIType.setTypeName(typeName);
        }

        else if (nodeName.equalsIgnoreCase(XMLConstants.GLOBAL_VARIABLE_UI)) {
        ...
        if (nmrOfInstancesAttr != null) {
          ...
          nmrOfInstances = Integer.valueOf(nmrOfInstancesAttr.getValue());
          initialStateUIType.setNmrOfInstances(nmrOfInstances.intValue());

        } else {
          initialStateUIType.setNmrOfInstances(InitialStateUIType.UNBOUNDED);

        }
        this.initializePropertiesInfoHashMap(initialStateUIType, child,
            typeName, categoryType);
        this.initializeInitialStateUIInstancesHashMap(initialStateUIType,
            child, typeName, nodeName);
        ...
        for (InitialStateUIProperty initialStateUIProperty : initialStateUIType
          ...
          if (langSet.contains(InitialStateUIProperty.No_Lang_Attr_Given)) {
            langSet.remove(InitialStateUIProperty.No_Lang_Attr_Given);
          }
  ...
  private void initializePropertiesInfoHashMap(
      InitialStateUIType initialStateUIType, Node typeNode, String typeName,
    ...
    case Global: {
      initializePropertiesInfoHashMapGlobals(initialStateUIType, typeNode,
          typeName, categoryType);
    }
    }
  ...
  private void initializePropertiesInfoHashMapGlobals(
    ...
    if (typeNode instanceof Element) {
      ...
      if (initialStateUIProperty.isPropertyValid())
        propertiesInfoHashMap.put(initialStateUIProperty.getPropertyName(),
  ...
  private void initializePropertiesInfoHashMapObjektsEvents(
    ...
    if (categoryType.equals(CategoryType.Agent)) {
      initializeProperties(propertyNodeList, typeName, propertiesInfoHashMap,
          categoryType, AgentType.Objective);
      propertyNodeList = this.simulationDescription.getNodeList(PX
          + XMLConstants.SUBJECTIVE_PROPERTY_UI, typeNode);
      propertiesInfoHashMap = initializeProperties(propertyNodeList, typeName,
          propertiesInfoHashMap, CategoryType.Agent, AgentType.Subjective);
    } else {
      initializeProperties(propertyNodeList, typeName, propertiesInfoHashMap,
          categoryType, null);
    }
  ...
  private HashMap<String, InitialStateUIProperty> initializeProperties(
    ...
    for (int i = 0; i < propertyNodeList.getLength(); i++) {
      ...
      propertiesInfoHashMap = processPropertyNode(child,
          initialStateUIProperty, typeName, categoryType, propertiesInfoHashMap);
  ...
  private InitialStateUIProperty initializeProperty(Element propertyElement,
    ...
    if (initialStateUIProperty.isPropertyValid()) {
      ...
      if ((propertyClass.equals(long.class))
        ...
        initializeMaxMinForProperty(initialStateUIProperty, categoryType,
            typeName);
      }
    }
  ...
  private void initializeMaxMinForProperty(
      InitialStateUIProperty initialStateUIProperty, CategoryType categoryType,
      String typeName) {
    String packageAndClassName = null;
    String propertyName = initialStateUIProperty.getPropertyName();

    packageAndClassName = initializePackageAndClassName(typeName, categoryType,
        initialStateUIProperty.getAgentType());
  ...
  private String initializePropertyName(Element propertyElement,
    ...
    if (categoryType.equals(CategoryType.Global)) {
      propertyName = propertyElement.getAttributeNode(XMLConstants.VARIABLE)
          .getValue();
    } else {
      propertyName = propertyElement.getAttributeNode(XMLConstants.PROPERTY)
          .getValue();
    }
    return propertyName;
  }
  ...
  private Double initializeSliderStepSize(Element propertyElement) {
    ...
    if (sliderStepSizeAttribute != null) {
      ...
      sliderStepSize = new Double((Double.parseDouble(sliderStepSizeString)));
    } else {
  ...
  private Long initializeInputFieldLength(Element propertyElement) {
    ...
    if (inputFieldLengthAttribute != null) {
      ...
      inputFieldLength = new Double(Double.parseDouble(nmrOfInstancesString))
          .longValue();
    } else {
  ...
  private void initializeInitialStateUIEventInstancesHashMap(
    ...
    for (EnvironmentEvent environmentEvent : eventList) {
      ...
      if (eventClassName.equalsIgnoreCase(typeName)) {
        ...
        environmentEventIndex = new Long(eventList.indexOf(environmentEvent));
        eventInstanceId = environmentEvent.getId();
        environmentEventInstanceName = environmentEvent.getName();
        initialStateUIInstance = new InitialStateUIInstance(eventInstanceId);
        initialStateUIInstance.setName(environmentEventInstanceName);

        this.initializeInitialStateUIinstancePropertiesHashMap(
            initialStateUIType, initialStateUIInstance, typeNode, typeName,
            environmentEvent);
        initialStateUIType.getInstancesHashMap().put(environmentEventIndex,
  ...
  private void initializeInitialStateUIObjektsInstancesHashMap(
    ...
    Iterator<Objekt> typeNameObjektListIterator = typeNameObjektList.iterator();
    ...
    while (typeNameObjektListIterator.hasNext()) {
      ...
      initialStateUIType.getInstancesHashMap().put(typeNameObjektInstanceId,
          initialStateUIInstance);
  ...
  private void initializePropertyUnit(
    ...
    Node unitNode = this.simulationDescription.getNode(PX + XMLConstants.UNIT,
        propertyNode);
  ...
  private void initializeLanguagePropertyHintTextHashMap(
    ...
    Node hintNode = this.simulationDescription.getNode(PX + XMLConstants.HINT,
        propertyNode);
    ...
    for (int i = 0; i < textNodeList.getLength(); i++) {
      ...
      if (child instanceof Element) {
        ...
        langAttribute = textElement.getAttributeNode(XMLConstants.XML_LANG);
  ...
  private void initializeLanguagePropertyLabelTextHashMap(
    ...
    Node labelNode = this.simulationDescription.getNode(
        PX + XMLConstants.LABEL, propertyNode);
    NodeList textNodeList = this.simulationDescription.getNodeList(PX
    ...
    for (String typeName : this.initialStateUIHashMap
      ...
      case Global: {
        ...
        String selectedLanguageType = this.GUIComponent
            .getInitialStateUIBottomPanel().getSelectedLanguageType();
  ...
  public void getPropertiesListForSelectedType(String selectedType) {
    ...
    initialStateUI.setSelectedTypePropertiesList(selectedTypePropertiesList);
    ...
    for (InitialStateUIProperty initialStateUIProperty : propertiesInfoHashMap
      ...
      if (initialStateUIProperty.getPropertyName().equalsIgnoreCase(
        ...
        for (int i = 0; i < selectedTypePropertiesList.size(); i++) {
          ...
          if (initialStateUIProperty2.getPropertyName().equalsIgnoreCase(
              PropertyNameConstants.INSTANCE_NAME)) {
            int index = selectedTypePropertiesList
                .indexOf(initialStateUIProperty2);
            selectedTypePropertiesList.remove(index);
            selectedTypePropertiesList.add(index, initialStateUIProperty);
  ...
  public void getPropertiesValuesForSelectedType(String selectedType) {
    ...
    initialStatePropertiesNamesList.add(PropertyNameConstants.INSTANCE_NAME);
    ...
    for (Long key : initialStateInstancesHashMap.keySet()) {
      ...
      initialStateUI.setInitialStatePropertiesData(initialStatePropertiesData);

      /*
       * For Rest of the Properties for the selected Type
       */
      getInformationForRemainingProperties(selectedType, key, initialStateUI,
          previousKey);
      previousKey = key;
  ...
  private void getInformationForRemainingProperties(String selectedType,
    ...
    for (Object propertyName : propertiesValuesHashMap.keySet()) {
      ...
      if (!propertiesNamesList.contains(propertyName)) {
        ...
        initialStateUIPropertyValue = propertiesValuesHashMap.get(propertyName);

        propertiesDataList.add(initialStateUIPropertyValue.getPropertyValue());
...

        {
          ...
          propertiesDataList
              .add(initialStateUIPropertyValue.getPropertyValue());
        }
      }
  ...
  public void editPropertiesNamesHints(String selectedLanguage) {
    ...
    for (InitialStateUIProperty initialStateUIProperty : selectedTypePropertiesList) {
      ...
      propertyLabel = getPropertyLabel(initialStateUIProperty, selectedLanguage);
      if (initialStateUIProperty.getAgentType() != null) {
        if (initialStateUIProperty.getAgentType().equals(AgentType.Subjective)) {
          propertyLabel = propertyLabel + "(*)";
        }
      }

      initialStatePropertiesNamesList.add(propertyLabel);
      propertyHint = getPropertyHint(initialStateUIProperty, selectedLanguage);
  ...
  private String getPropertyHint(InitialStateUIProperty initialStateUIProperty,
    ...
    if (languagePropertyLabelHintHashMap != null) {
      ...
      if (languagePropertyLabelHintHashMap.containsKey(selectedLanguage)) {
        propertyHint = languagePropertyLabelHintHashMap.get(selectedLanguage);
      } else {
  ...
  private String getPropertyLabel(
      InitialStateUIProperty initialStateUIProperty, String selectedLanguage) {
    String propertyName;
    String propertyLabel;
    HashMap<String, String> languagePropertyLabelTextHashMap = initialStateUIProperty
        .getLanguagePropertyLabelTextHashMap();
    ...
    if (languagePropertyLabelTextHashMap != null) {
      ...
      if (languagePropertyLabelTextHashMap.containsKey(selectedLanguage)) {
        propertyLabel = languagePropertyLabelTextHashMap.get(selectedLanguage);
      } else {
  ...
  public void updateInitialStateUIHashMap(String selectedObjectType,
    ...
    if (selectedObjectType.equals(CategoryType.Global.name())) {
      ...
      InitialStateUIPropertyValue initialStateUIPropertyValue = initialStateUIInstance
          .getInstancePropertiesValuesHashMap().get(changedPropertyName);
      if (initialStateUIPropertyValue != null) {
        initialStateUIPropertyValue.setPropertyValue(changedPropertyValue);
      }
  ...
  public Object updateValueInActualObjects(Entity changedEntity,
    ...
    } else // Case of Globals
    {
      ...
      packageAndClassName = initializePackageAndClassName(CategoryType.Global
          .name(), CategoryType.Global, null);
    ...
    try {
      ...
      Object o = setter.invoke(setterInvokingObject, changedPropertyValue);
    } catch (IllegalArgumentException e) {
  ...
  public boolean isPresent(Long newInstanceID) {
    InitialStateUIHashMap initialStateUIHashMap = this.getInitialStateHashMap();
    for (InitialStateUIType initialStateUIType : initialStateUIHashMap
        .getInitialStateTypeshashMap().values()) {
      for (Long instanceHashMapKey : initialStateUIType.getInstancesHashMap()
          .keySet()) {
  ...
  public void copyInitialStateUIInstance(Long newInstanceID,
    ...
    case OBJECT_LIST: {
      ...
      this.addInitialStateUIEditedInformation(UpdateType.COPY, selectedType,
          newInstanceID);
      break;
    }
    case EVENT_LIST: {
  ...
  public void deleteInitialStateUIInstance(Long selectedInstanceID,
    ...
    this.addInitialStateUIEditedInformation(UpdateType.DELETE, selectedType,
        selectedInstanceID);
    InitialStateUIType initialStateUIType = this.getInitialStateHashMap()
        .getTypeStructure(selectedType);
  ...
  private void updateEventsKeyInHashMap(Long deletedInstanceKey) {
    ...
    for (InitialStateUIType initialStateUIType : this.initialStateUIHashMap
        .getInitialStateTypeshashMap().values()) {
      if (initialStateUIType.getCategoryType().equals(CategoryType.Event)) {
        for (Long key : initialStateUIType.getInstancesHashMap().keySet()) {
  ...
  public boolean updateInitialStateUIHashMap(String selectedType) {
    InitialStateUIType initialStateUIType = this.initialStateUIHashMap
        .getTypeStructure(selectedType);
    HashMap<String, Object> createdInstancePropertiesValues = this.GUIComponent
    ...
    for (String propertyName : createdInstancePropertiesValues.keySet()) {
      ...
      if (propertyName.equalsIgnoreCase(PropertyNameConstants.INSTANCE_ID)) {
        ...
        case EVENT_LIST: {
          initialStateUIInstance = new InitialStateUIInstance(createdInstanceID);
    ...
    if (!flagerror) {
      for (String propertyName : createdInstancePropertiesValues.keySet()) {
        ...
        if (propertyName.equalsIgnoreCase(PropertyNameConstants.INSTANCE_NAME)) {
          ...
          initialStateUIInstance.getInstancePropertiesValuesHashMap().put(
              propertyName, initialStateUIPropertyValue);
        }
      }
      ...
      case OBJECT_LIST: {
        initialStateUIType.addInitialStateUIInstance(createdInstanceID,
            initialStateUIInstance);
        this.addInitialStateUIEditedInformation(UpdateType.NEW, selectedType,
            createdInstanceID);
        break;
      }
      case EVENT_LIST: {
        ...
        initialStateUIType.addInitialStateUIInstance(createdEventHashMapKey,
            initialStateUIInstance);
        this.addInitialStateUIEditedInformation(UpdateType.NEW, selectedType,
            createdEventHashMapKey);
        break;
  ...
  private int getCurEventsSize() {
    ...
    for (InitialStateUIType initialStateUIType : this.initialStateUIHashMap
        .getInitialStateTypeshashMap().values()) {
      if (initialStateUIType.getCategoryType().equals(CategoryType.Event)) {
        curEventsSize += initialStateUIType.getInstancesHashMap().size();
      }
  ...
  private void deleteActualObject(Long instanceID, CategoryType categoryType) {
    ...
    case Event: {
      ...
      if (instanceID < this.initialState.getEvents().size()) {
        ...
        EnvironmentEvent environmentEvent = this.initialState.getEvents().get(
            instanceID.intValue());
  ...
  private void editActualObject(ObjektUpdate objektUpdate,
    ...
    initialStateUIProperty = initialStateUIType.getPropertiesInfoHashMap().get(
        updatedPropertyName);
    ...
    if (!typeName.equals(CategoryType.Global.name())) {
      ...
      if (initialStateUIInstance != null) {
        ...
        updatedPropertyValue = getUpdatedPropertyValue(initialStateUIInstance,
            updatedPropertyName);
        ...
        typeNameEntity = getTypeNameEntity(categoryType, instanceID, agentType);

        typeNameEntity = (Entity) updateValueInActualObjects(typeNameEntity,
            updatedPropertyName, updatedPropertyValue, propertyClass);
      }
    } else {
  ...
  private Class<?> getUpdatedPropertyClass(
      InitialStateUIProperty initialStateUIProperty, Object updatedPropertyValue) {
  ...
  private Object getUpdatedPropertyValue(
      InitialStateUIInstance initialStateUIInstance, String updatedPropertyName) {
    ...
    if (!updatedPropertyName
      ...
      initialStatePropertyValue = initialStateUIInstance
          .getInstancePropertiesValuesHashMap().get(updatedPropertyName);
  ...
  private Entity getTypeNameEntity(CategoryType categoryType, Long instanceID,
      AgentType agentType) {
    Entity typeNameEntity = null;
    switch (categoryType) {
    case Agent: {
      ...
      case Subjective: {
        typeNameEntity = this.getInitialState().getAgentSubjectById(instanceID);
        break;
      }
      }
  ...
  private void createObjekt(String typeName, Long instanceID,
    ...
    case Agent: {
      packageAndClassName = initializePackageAndClassName(typeName,
          categoryType, AgentType.Objective);
      AgentObject agentObject = (AgentObject) createEntity(packageAndClassName,
          initialStateUIInstance, initialStateUIType, AgentType.Objective);

      packageAndClassName = initializePackageAndClassName(typeName,
          categoryType, AgentType.Subjective);
      AgentSubject agentSubject = (AgentSubject) createEntity(
          packageAndClassName, initialStateUIInstance, initialStateUIType,
          AgentType.Subjective);

      agentSubject
          .addInternalEvent(new aors.model.intevt.EachSimulationStep(1));
    ...
    case Object: {
      packageAndClassName = initializePackageAndClassName(typeName,
          categoryType, null);
      createdEntity = createEntity(packageAndClassName, initialStateUIInstance,
          initialStateUIType, null); // AgentType
      // Null
      // for
      // Objects
    ...
    case Event: {
      ...
      // for
      // Events
      this.initialState.addEnvironmentEvent((EnvironmentEvent) createdEntity);
      break;
    }
    }
  ...
  private Entity createEntity(String packageAndClassName,
    ...
    Class<?>[] parameterTypeList = createdObjektConstructor.getParameterTypes();
    Object[] params = new Object[parameterno];

    params = initializeParams(initialStateUIInstance, parameterTypeList, params);
  ...
  private Entity instantiateEntity(Constructor<?> createdObjektConstructor,
    ...
    try {
      ...
      case Object: {
        createdEntity = (Objekt) createdObjektConstructor.newInstance(params);
        break;
      }
      case Event:
    ...
    for (String propertyName : instancePropertiesValuesHashMap.keySet()) {
      ...
      if (initialStateUIType.getCategoryType().equals(CategoryType.Agent)) {
        if (initialStateUIProperty.getAgentType().equals(agentType)) {
          propertyClass = initialStateUIProperty.getPropertyClass();
          createdEntity = (Entity) updateValueInActualObjects(createdEntity,
              propertyName, propertyValue, propertyClass);
        }
